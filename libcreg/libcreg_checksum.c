...
{
	libcreg_aligned_t *aligned_buffer_iterator = NULL;
	uint8_t *buffer_iterator                   = NULL;
	static char *function                      = "libcreg_checksum_calculate_little_endian_xor32";
	libcreg_aligned_t value_aligned            = 0;
	 ...
	 */
	if( ( ( sizeof( libcreg_aligned_t ) % 4 ) == 0 )
	 && ( size > ( 2 * sizeof( libcreg_aligned_t ) ) ) )
	{
		/* Align the buffer iterator
		 */
		alignment_size = (uint8_t) ( (intptr_t) buffer_iterator % sizeof( libcreg_aligned_t ) );

		if( alignment_size > 0 )
		{
			byte_size = sizeof( libcreg_aligned_t ) - alignment_size;
		...
		aligned_buffer_iterator = (libcreg_aligned_t *) buffer_iterator;
		 ...
		 */
		while( size > sizeof( libcreg_aligned_t ) )
		{
			value_aligned ^= *aligned_buffer_iterator;

			aligned_buffer_iterator++;

			size -= sizeof( libcreg_aligned_t );
		}
		...
		if( alignment_size > 0 )
		{
			...
			alignment_count = ( sizeof( libcreg_aligned_t ) - alignment_size ) * 8;
		 ...
		 */
		byte_size = (uint8_t) sizeof( libcreg_aligned_t );
