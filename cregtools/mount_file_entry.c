     ...
     libcerror_error_t **error )
{
	...
	if( ( type != MOUNT_FILE_ENTRY_TYPE_KEY )
	...
	*file_entry = memory_allocate_structure(
	...
	( *file_entry )->creg_key   = creg_key;
	( *file_entry )->creg_value = creg_value;
     ...
     libcerror_error_t **error )
{
	...
	if( *file_entry != NULL )
	{
		...
		if( ( *file_entry )->creg_key != NULL )
		{
			if( ( *file_entry )->type != MOUNT_FILE_ENTRY_TYPE_VALUE )
			{
				if( libcreg_key_free(
				     &( ( *file_entry )->creg_key ),
				     error ) != 1 )
		...
		if( ( *file_entry )->creg_value != NULL )
		{
			if( libcreg_value_free(
			     &( ( *file_entry )->creg_value ),
			     error ) != 1 )
			{
				libcerror_error_set(
				 error,
...
int mount_file_entry_get_sub_file_entry_by_index(
     ...
     libcerror_error_t **error )
{
	libcreg_key_t *sub_creg_key               = NULL;
	...
	else
	{
		...
		else
		{
			...
			sub_creg_key = file_entry->creg_key;
		}
	}
