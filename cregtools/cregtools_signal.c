...
void (*cregtools_signal_signal_handler)( cregtools_signal_t ) = NULL;
             ...
             cregtools_signal_t signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( cregtools_signal_signal_handler != NULL )
			{
				cregtools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
     ...
     libcerror_error_t **error )
{
	...
	cregtools_signal_signal_handler = NULL;
