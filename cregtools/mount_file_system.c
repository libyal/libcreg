     ...
     libcerror_error_t **error )
{
	...
	if( file_system->creg_file != NULL )
	{
		if( libcreg_file_signal_abort(
		     file_system->creg_file,
		     error ) != 1 )
		{
			libcerror_error_set(
			 error,
     ...
     libcerror_error_t **error )
{
	...
	file_system->creg_file = creg_file;
     ...
     libcerror_error_t **error )
{
	...
	*creg_file = file_system->creg_file;
     ...
     libcerror_error_t **error )
{
	...
	if( mount_path_string_copy_to_key_path(
	     path,
	     path_length,
	     &key_path,
	     &key_path_size,
	     error ) != 1 )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to copy path to key path.",
		 function );
     ...
     libcerror_error_t **error )
{
	...
	if( mount_path_string_copy_from_key_path(
	     filename,
	     filename_size,
	     key_name,
	     key_name_size - 1,
	     error ) != 1 )
	{
		libcerror_error_set(
		 ...
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
		 LIBCERROR_RUNTIME_ERROR_COPY_FAILED,
		 "%s: unable to copy filename from the key name.",
		 function );
     ...
     libcerror_error_t **error )
{
	     ...
	     error ) != 1 )
	{
		libcerror_error_set(
		 ...
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
		 LIBCERROR_RUNTIME_ERROR_COPY_FAILED,
		 "%s: unable to copy filename from the value name.",
		 function );
