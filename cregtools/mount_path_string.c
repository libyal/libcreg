...
/* Copies the path from a comparable key path
 * Returns 1 if successful or -1 on error
 */
     ...
     libcerror_error_t **error )
{
	...
	static char *function                        = "mount_path_string_copy_from_key_path";
	...
	if( ( key_path_length == 0 )
	 || ( key_path_length > (size_t) ( SSIZE_MAX - 1 ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
		 LIBCERROR_ARGUMENT_ERROR_VALUE_OUT_OF_BOUNDS,
		 "%s: invalid key path length value out of bounds.",
		 function );
	 ...
	 */
	if( key_path_length > (size_t) ( ( SSIZE_MAX - 1 ) / ( sizeof( system_character_t ) * 10 ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	safe_path_size = ( key_path_length * 10 ) + 1;
	...
	while( key_path_index < key_path_length )
	{
#if defined( HAVE_WIDE_SYSTEM_CHARACTER )
		result = libuna_unicode_character_copy_from_utf16(
		          &unicode_character,
		          (libuna_utf16_character_t *) key_path,
		          key_path_length,
		          &key_path_index,
		          error );
#else
		result = libuna_unicode_character_copy_from_utf8(
		          &unicode_character,
		          (libuna_utf8_character_t *) key_path,
		          key_path_length,
		          &key_path_index,
		          error );
#endif
		if( result != 1 )
		{
			libcerror_error_set(
			 ...
			 "%s: unable to copy Unicode character from key path.",
			 function );
...
/* Copies the path to a comparable key path
 * Returns 1 if successful or -1 on error
 */
     ...
     libcerror_error_t **error )
{
	...
	static char *function                        = "mount_path_string_copy_to_key_path";
	libuna_unicode_character_t unicode_character = 0;
	...
	while( path_index < path_length )
	{
		...
		{
			unicode_character = (libuna_unicode_character_t) LIBCREG_SEPARATOR;
		}
		else if( unicode_character == (libuna_unicode_character_t) ESCAPE_CHARACTER )
		{
