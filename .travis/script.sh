...
then
	docker run -t -v "${PWD}:/libcreg" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libcreg && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libcreg;
